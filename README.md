...
### 共識
- 遊戲主題為「西城之鑰：愛的時光漫遊」，以台北西門町歷史文化為背景，結合時空穿越解謎元素。
- 遊戲章節由原本6章擴展為8章，每章節包含前半段故事、簡單謎題、後半段故事、複雜謎題、道具收集與結論。
- 主角命名為「林宇軒」，現代西門少年，熱愛嘻哈與街頭文化，由阿公阿嬤帶大。
- 謎題型態支援文字輸入、數字輸入、選擇題（choice）、拖拉互動（drag）與相機拍照。
- 每章節一個主線道具，共8個，最終拼成完整信件。

### 架構
- 技術架構為 HTML5 + CSS3 + JavaScript（純前端），部署於 GitHub Pages，並與 GitHub 倉庫同步。
- 遊戲資料存儲於 `data/gameData.json`，包含遊戲標題、副標題、主題、設定、章節內容與結局。
- 章節內容包含故事、謎題、圖片與結論，謎題型態與答案格式明確，避免混用。

### 底層邏輯或規則
- 謎題答案必須能從故事文本中推理得出，避免冷知識突襲。
- 填充題為純文字或純數字，選擇題為範圍選項，拖拉題為正確拖曳，拍照題為互動體驗。
- 每章節的謎題型態與答案格式需嚴格遵循，確保遊戲體驗一致性。

### 2025-05-29 開發紀錄

- 針對所有主要程式檔案（index.html、main.js、style.css、data/gameData.json）加上清楚的中文區塊註解，並補充資料結構說明檔，方便初學者閱讀與維護。
